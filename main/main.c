...
uCanvas_Animation_task_handle_t handle;
...
void circle_animation_task(uCanvas_universal_obj_t* obj){
    int proportion = 3;
    int delay1 = 14;
    int delay2 = 5;
    while (1)
    {
        if(obj->state == false){
            ...
            {
                obj->properties.position.y += 1;
                uCanvas_Delay(delay2);
            }
            uCanvas_Delay(delay1);
            for (int i = 0; i < proportion; i++)
            {
                obj->properties.position.y -= 1;
                uCanvas_Delay(delay2);
...
void controller_callback1(void){
    printf("Jump animation done\r\n");
    counter++;
    sprintf(textbox1->text,"jumped %d times",counter);
    // uCanvas_Pause_Task(handle);
    
}
void controller_callback2(void){
    printf("User Input detected\r\n");
    uCanvas_Resume_Task(handle);
}

void get_live_coordinates(uCanvas_universal_obj_t* obj){
    while (1)
    {
        // printf("y location %d\r\n",obj->properties.position.y);
        sprintf(textbox2->text,"y:%d",obj->properties.position.y);
        uCanvas_Delay(1);
    }
}
...
void ucanvas_app_setup(){
    int horizontal_position = 64, 
        ...
        size                = 5;
    uCanvas_universal_obj_t* circle = New_uCanvas_2DCircle(horizontal_position,vertical_position,size);
    textbox1 = New_uCanvas_2DTextbox(" ",0,3);
    textbox2 = New_uCanvas_2DTextbox(" ",0,15);
    circle->properties.fill = FILL;
    circle->state = false;
    controller_properties_t* player_properties = (controller_properties_t*)malloc(sizeof(controller_properties_t));

    uCanvas_add_control_gpio(player_properties  ,5);
    uCanvas_set_gravity     (player_properties  ,-2.2);
    uCanvas_set_jump_height (player_properties  ,13.4);
    uCanvas_set_floor_level (player_properties  ,vertical_position);
    
    player_properties->user_callback_post_jump = controller_callback1;
    player_properties->user_callback_pre_jump  = NULL;

    uCanvas_attach_type1_controller_script(circle,player_properties);
    uCanvas_Add_Task(circle_animation_task,circle,0);
    handle =  uCanvas_Add_Task(get_live_coordinates,circle,0);
    uCanvas_Pause_Task(handle);
}
