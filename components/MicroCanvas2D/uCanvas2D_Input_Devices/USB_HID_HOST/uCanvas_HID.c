...
static uCanvas_Input_HID_Device_t* InputDeviceHandle = NULL;
...
static void key_event_callback(key_event_t* key_event) {
    if (!InputDeviceHandle || !key_event) {
        printf("Invalid InputDeviceHandle\r\n");
        return;
    }
    ...
    // if (KEY_STATE_PRESSED == key_event->state) {
        if (hid_keyboard_get_char(key_event->modifier,key_event->key_code, &key_char)) {
            key_event->key_char = key_char; 
        }
        if(InputDeviceHandle->EventListenerCallback){ 
            InputDeviceHandle->EventListenerCallback(INPUT_DEVICE_KEYBOARD,key_event,InputDeviceHandle->arg);
        }
...
void USB_HID_Monitor_Task(void*arg)
{
    /*
...
void USB_HID_Setup(void){
    ...
    */
    ...
    task_created = xTaskCreatePinnedToCore(USB_HID_Monitor_Task,
                                           "usb_events",
                                           4096,
                                           xTaskGetCurrentTaskHandle(),
                                           2, NULL, 0);
...
void uCanvas_Get_HID_Device(uCanvas_Input_HID_Device_t* InputDeviceInstance){
    InputDeviceInstance->uCanvas_Input_HID_Device_Init = USB_HID_Setup;
    InputDeviceInstance->uCanvas_Input_HID_Device_DeInit = USB_HID_DeInit;
    InputDeviceInstance->listener_idx = 0;
